...
function ParticleField() {
  ...
  useEffect(() => {
    ...
    const PARTICLE_COUNT = 80; // Optimized for performance
    ...
    function draw() {
      ...
      // Add a slight neon glow to the particles
      ctx.shadowBlur = 8;
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < 130) {
            ...
            ctx.strokeStyle = `rgba(0,245,255,${(1 - d / 130) * 0.15})`;
            ctx.lineWidth = 0.6;
    ...
    <div className="holo-wrap">
      ...
      <div className="hex-frame">
        <svg viewBox="0 0 200 200" width="200" height="200">
          <polygon points="100,10 185,55 185,145 100,190 15,145 15,55" fill="none" stroke="rgba(0,245,255,0.35)" strokeWidth="1" strokeDasharray="8 4" className="hex-spin" />
          <polygon points="100,25 172,66 172,134 100,175 28,134 28,66" fill="none" stroke="rgba(168,85,247,0.25)" strokeWidth="0.5" strokeDasharray="4 8" className="hex-spin-rev" />
        </svg>
      </div>
      ...
      <svg className="data-arcs" viewBox="0 0 300 300" width="300" height="300">
        <circle cx="150" cy="150" r="120" fill="none" stroke="rgba(0,245,255,0.15)" strokeWidth="1" strokeDasharray="20 8" className="arc-spin" />
        <circle cx="150" cy="150" r="100" fill="none" stroke="rgba(168,85,247,0.2)" strokeWidth="1" strokeDasharray="12 16" className="arc-spin-rev" />
        <circle cx="150" cy="150" r="80" fill="none" stroke="rgba(0,245,255,0.1)" strokeWidth="0.5" strokeDasharray="6 20" className="arc-spin" />
      </svg>
      {["tl", "tr", "bl", "br"].map((pos) => (
  ...
  useEffect(() => {
    ...
    bootInterval = setInterval(() => {
      ...
      if (lineIdx >= bootSeq.length) {
        ...
        transitionTimeout = setTimeout(() => setPhase(1), 500);
      }
    }, 220);
  ...
  useEffect(() => {
    ...
    iv = setInterval(() => {
      if (i < fullText.length) {
        ...
        clearInterval(iv);
        setPhase(3);
        completionTimeout = setTimeout(onComplete, 1800);
      }
    }, 25);
...
export default function Page() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/buildlio", {
        ...
        body: JSON.stringify({ input, buildType }),
      });

      const data = await res.json();
      const full = data.text || data.error || "Neural link established. Processing architecture...";
  ...
  // Safe global styling injection for Next.js App Router
  const globalStyles = `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bl-root { min-height: 100vh; background: var(--bg); color: var(--text); font-family: var(--font-sans), sans-serif; overflow-x: hidden; position: relative; }
    
    /* Grid Background */
    .bl-root::before { content: ''; position: fixed; inset: 0; background-image: linear-gradient(rgba(0,245,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,245,255,0.03) 1px, transparent 1px); background-size: 48px 48px; pointer-events: none; z-index: 1; }
    .bl-root::after { content: ''; position: fixed; inset: 0; background: radial-gradient(ellipse 80% 60% at 50% 0%, rgba(168,85,247,0.06) 0%, transparent 60%), radial-gradient(ellipse 60% 40% at 80% 100%, rgba(0,245,255,0.04) 0%, transparent 50%); pointer-events: none; z-index: 1; }

    /* Intro */
    .intro-shell { position: fixed; inset: 0; background: var(--bg); display: flex; align-items: center; justify-content: center; z-index: 300; flex-direction: column; gap: 0; }
    .boot-terminal { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 640px; background: rgba(10,10,26,0.97); border: 1px solid rgba(0,245,255,0.3); border-radius: 12px; padding: 24px 28px; font-family: var(--font-mono), monospace; font-size: 13px; transition: opacity 0.6s ease, transform 0.6s ease; box-shadow: 0 0 80px rgba(0,245,255,0.08), inset 0 0 40px rgba(0,245,255,0.02); }
    .boot-terminal.boot-exit { opacity: 0; transform: translate(-50%, -54%) scale(0.96); pointer-events: none; }
    .boot-header { display: flex; justify-content: space-between; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid rgba(0,245,255,0.15); }
    .boot-tag { color: var(--c); font-weight: 700; letter-spacing: 3px; }
    .boot-ver { color: var(--muted); font-size: 11px; }
    .boot-line { color: var(--text); margin-bottom: 6px; opacity: 0; animation: bootFadeIn 0.3s ease forwards; line-height: 1.5; }
    .boot-line:last-child { color: var(--g); }
    .boot-prompt { color: var(--c); margin-right: 8px; }
    .boot-cursor { display: inline-block; width: 8px; height: 14px; background: var(--c); animation: cyberBlink 0.8s step-end infinite; vertical-align: middle; margin-left: 4px; }
    @keyframes bootFadeIn { to { opacity: 1; } }

    .intro-main { display: flex; flex-direction: column; align-items: center; gap: 64px; opacity: 0; transform: scale(0.92); transition: opacity 0.8s cubic-bezier(0.23,1,0.32,1), transform 0.8s cubic-bezier(0.23,1,0.32,1); pointer-events: none; }
    .intro-main.intro-visible { opacity: 1; transform: scale(1); pointer-events: all; }
    ...
    .scanner-ring { position: absolute; border-radius: 50%; border: 1px solid rgba(0,245,255,0.12); animation: scanPulse 4s ease-in-out infinite; }
    ...
    @keyframes scanPulse { 0%, 100% { opacity: 0.15; border-color: rgba(0,245,255,0.12); } 50% { opacity: 0.5; border-color: rgba(0,245,255,0.3); } }
    ...
    .core-sphere { width: 120px; height: 120px; border-radius: 50%; background: radial-gradient(circle at 38% 30%, #67e8f9 0%, #6366f1 45%, #1e0a3c 100%); box-shadow: 0 0 60px rgba(103,232,249,0.5), 0 0 120px rgba(168,85,247,0.3), inset 0 0 50px rgba(255,255,255,0.08); position: relative; z-index: 10; overflow: hidden; }
    .core-sphere.live { animation: spherePulse 3.5s ease-in-out infinite; }
    @keyframes spherePulse { 0%,100% { box-shadow: 0 0 60px rgba(103,232,249,0.5), 0 0 120px rgba(168,85,247,0.3); } 50% { box-shadow: 0 0 100px rgba(103,232,249,0.8), 0 0 200px rgba(168,85,247,0.5); } }
    ...
    .od-0 { top: 8%; left: 50%; animation: orbitDot0 6s linear infinite; } .od-1 { top: 50%; right: 8%; animation: orbitDot1 7s linear infinite; } .od-2 { bottom: 8%; left: 50%; animation: orbitDot2 8s linear infinite; } .od-3 { top: 50%; left: 8%; animation: orbitDot3 9s linear infinite; background: var(--v); box-shadow: 0 0 10px var(--v); } .od-4 { top: 18%; right: 18%; animation: orbitDot4 5s linear infinite; width: 4px; height: 4px; } .od-5 { bottom: 18%; left: 18%; animation: orbitDot5 11s linear infinite; width: 4px; height: 4px; background: var(--v); }
    @keyframes orbitDot0 { 0%,100% { transform: translate(-50%, 0); } 50% { transform: translate(-50%, -8px); } } @keyframes orbitDot1 { 0%,100% { transform: translate(0, -50%); } 50% { transform: translate(8px, -50%); } } @keyframes orbitDot2 { 0%,100% { transform: translate(-50%, 0); } 50% { transform: translate(-50%, 8px); } } @keyframes orbitDot3 { 0%,100% { transform: translate(0, -50%); } 50% { transform: translate(-8px, -50%); } } @keyframes orbitDot4 { 0%,100% { transform: translate(0, 0); } 50% { transform: translate(6px, -6px); } } @keyframes orbitDot5 { 0%,100% { transform: translate(0, 0); } 50% { transform: translate(-6px, 6px); } }

    .bracket { position: absolute; width: 16px; height: 16px; z-index: 15; }
    .br-tl { top: -8px; left: -8px; border-top: 2px solid var(--c); border-left: 2px solid var(--c); } .br-tr { top: -8px; right: -8px; border-top: 2px solid var(--c); border-right: 2px solid var(--c); } .br-bl { bottom: -8px; left: -8px; border-bottom: 2px solid var(--c); border-left: 2px solid var(--c); } .br-br { bottom: -8px; right: -8px; border-bottom: 2px solid var(--c); border-right: 2px solid var(--c); }
    ...
    .hud-tl { top: 28px; left: 28px; } .hud-tr { top: 28px; right: 28px; } .hud-bl { bottom: 28px; left: 28px; } .hud-br { bottom: 28px; right: 28px; }
    @keyframes cornerPulse { 0%,100% { opacity: 0.4; } 50% { opacity: 0.8; } }

    /* Main UI */
    .bl-hud { position: fixed; top: 0; left: 0; right: 0; height: 56px; background: rgba(10,10,26,0.9); border-bottom: 1px solid var(--border); backdrop-filter: blur(24px); z-index: 100; display: flex; align-items: center; padding: 0 28px; font-family: var(--font-mono), monospace; font-size: 12px; letter-spacing: 1.5px; color: var(--muted); }
    ...
    .bl-wrap { position: relative; z-index: 10; max-width: 1280px; margin: 0 auto; padding: 76px 28px 160px; }
    ...
    .bl-card { background: rgba(15,23,42,0.9); border: 1px solid rgba(0,245,255,0.18); border-radius: 18px; overflow: hidden; cursor: pointer; transition: all 0.3s cubic-bezier(0.23,1,0.32,1); text-align: left; box-shadow: 0 8px 32px rgba(0,0,0,0.5); position: relative; }
