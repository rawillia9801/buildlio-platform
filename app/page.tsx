...
CHANGELOG
- v5.0
* Splash: solid white → corner ripple → type “Hi!” (pause) → finish message → type “I’ll guide you step-by-step.” → choices float up
* Choice click: card sinks + ripple expands (“SPLOOSH”) without cheesy text
* Console tab: WHITE “build console” with buildlio> prompt (not black terminal)
* Pending prompt: after selection, if authed -> builder auto-send; if not -> after login auto-send
* Keeps: Builder UI, preview, export, logs, history, credits
...
(async () => {
// Pure white first — calm
await sleep(650);
if (!alive) return;

// Corner ripple appears (still white)
setPhase(1);
...
// Type "Hi!" first, pause, then continue
const hi = "Hi!";
const speed1 = 34;
...
await sleep(720); // slower, less rushed
onSelect(choice);
};
...
.caret-on { height:0.95em; border-right:3px solid rgba(0,0,0,0.45); animation: blink 0.95s step-end infinite; }
...
.corner-ripple-2 { animation-delay: 0.25s; border-color: rgba(0,0,0,0.055); }
.corner-ripple-3 { animation-delay: 0.50s; border-color: rgba(0,0,0,0.04); }
@keyframes corner {
0% { transform: translate(-50%,-50%) scale(0.35); opacity: 0; }
16% { opacity: 0.6; }
100% { transform: translate(-50%,-50%) scale(10.5); opacity: 0; }
}
...
.choice-card:hover { transform: translateY(-2px); box-shadow:0 14px 34px rgba(0,0,0,0.085); }
.choice-card:active { transform: translateY(1px); }

.choice-sink { transform: translateY(12px) scale(0.985) !important; box-shadow: 0 6px 18px rgba(0,0,0,0.05) !important; }
.choice-sink::after {
content:"";
position:absolute;
inset:-2px;
...
opacity:1;
background:
radial-gradient(circle at 50% 65%, rgba(0,0,0,0.10), rgba(255,255,255,0) 58%),
radial-gradient(circle at 50% 65%, rgba(0,0,0,0.07), rgba(255,255,255,0) 62%);
animation: cardRipple 640ms ease-out forwards;
}
@keyframes cardRipple {
0% { transform: translateY(0) scale(0.88); opacity: 0.20; }
35% { opacity: 0.30; }
100% { transform: translateY(8px) scale(1.26); opacity: 0; }
}

.choice-fade { opacity:0.35 !important; transform: translateY(8px) scale(0.99) !important; }
...
.sploosh-ring-2 { animation-delay: 110ms; border-color: rgba(0,0,0,0.085); }
.sploosh-ring-3 { animation-delay: 200ms; border-color: rgba(0,0,0,0.06); }
@keyframes ring {
0% { transform: translate(-50%,-50%) scale(0.2); opacity:0; }
12% { opacity:0.55; }
100% { transform: translate(-50%,-50%) scale(86); opacity:0; }
}
.sploosh-wash {
position:absolute;
inset:0;
...
<button onClick={onSignOut} className="text-xs text-zinc-400 hover:text-white transition">Sign out</button>
</>
) : (
...
{ role: "assistant", content: "Hi, I’m Buildlio. Tell me what you’re building — and I’ll design a polished, high-converting site with a real professional structure." },
]);
...
setBuildLogs((prev) => [
...prev,
{ timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }), message, type },
]);
};
...
// Small pause so it feels intentional, not rushed
const t = setTimeout(() => {
setHasAutoSent(true);
...
// Route intentionally (not rushed)
setTimeout(() => {
...
<div className="mb-8 inline-flex items-center gap-4"><div className="text-8xl">⬡</div></div>
<h1 className="text-7xl font-black tracking-[-3.5px] leading-[1.05] mb-6">
Prompt.<br />Build.<br />
<span className="bg-gradient-to-r from-cyan-400 via-violet-400 to-fuchsia-400 bg-clip-text text-transparent">Ship professional sites.</span>
</h1>
...
<button onClick={() => (user ? setView("builder") : setView("auth"))} className="px-14 py-6 bg-white text-black rounded-3xl font-black text-2xl hover:scale-105 active:scale-95 transition">
Start building free
</button>
</div>
</div>
...
<p className="text-center text-[10px] text-zinc-500 mt-4">Buildlio will generate a complete site when ready</p>
</div>
</>
)}
...
<span className={log.type === "success" ? "text-emerald-700" : log.type === "error" ? "text-red-600" : "text-zinc-800"}>
{log.message}
</span>
</div>
))
...
<button onClick={exportHTML} disabled={!snapshot} className="flex items-center gap-2 px-7 py-2.5 bg-white/5 hover:bg-white/10 rounded-2xl text-sm font-medium disabled:opacity-40 transition">
Export Full HTML
</button>
</div>
